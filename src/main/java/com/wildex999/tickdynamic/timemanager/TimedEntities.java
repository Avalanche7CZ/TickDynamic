...
public class TimedEntities extends TimedGroup {
    ...
    // Initialize a timed Tile Entity group, reading in the configuration if it
    // exists.
    ...
    public void loadConfig(boolean saveDefaults) {
        ...
        if (base != null && !mod.config.hasKey(configEntry, configKeySlicesMax))
            sliceMax = mod.config.get(base.configEntry, configKeySlicesMax, sliceMax).getInt();
        else
    ...
    public void setMinimumObjects(int min) {
        minimumObjects = min;
    }
    ...
    public void setMinimumTPS(float min) {
        minimumTPS = min;
    }
    ...
    public void setMinimumTime(float min) {
        minimumTime = min;
    }
    ...
    public int startUpdateObjects() {
        ...
        if (currentObjectIndex >= listSize)
            currentObjectIndex = 0;
        if (TickDynamicMod.debugTimer)
            System.out.println("Start (" + name + "). Update Offset: " + currentObjectIndex + " | " + "Updating: " + updateCount + " of " + listSize);
